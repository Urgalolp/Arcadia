...
>###### Vehicle Stat Blocks
>
>To aid in running adventures where vehicles engage in combat, undertake precise navigation, or face situations where their various capabilities become relevant, the
following section presents new rules and stat blocks for a spectrum of cars, motorcycles and aircraft.
...
A vehicle's travel pace determines how far it can move per hour and per day. A vehicle's movement-related components (described later in the stat block) determine
...
A vehicle component is destroyed and becomes unusable when it drops to 0 hit points. A vehicle is wrecked if its hull is destroyed. A vehicle doesn't have Hit Dice.
#### Damage Threshold
...
In chase scale, most commonly encountered vehicles occupy only one square. (Some especially large vehicles, such as ships or jumbo jets, might occupy more than one
